...
abstract class AbstractSchemaCompiler implements SchemaCompilerInterface
{
    ...
    final protected function compileColumn(ColumnBlueprint $col, TableBlueprint $table): string
    {
    ...
    {
        throw new ConfigurationException('Database extensions are supported only by postgres schema compiler.');
    }
    ...
    {
        if ($table->changedColumns() === []) {
            return [];
        }

        foreach ($table->changedColumns() as $column) {
            $this->validateColumnCharacterOptions($column);
        }

        throw new ConfigurationException('Changing existing columns is not supported by this schema compiler.');
    }
    ...
    public function compileAlterTableDropColumns(TableBlueprint $table): array
    {
        ...
        foreach ($table->droppedColumns() as $column) {
            $out[] = 'ALTER TABLE ' . $tableSql . ' DROP COLUMN ' . $this->quoteIdentifier($column);
        }

        return $out;
    }

    public function compileAlterTableRenameColumns(TableBlueprint $table): array
    {
        $tableSql = $this->quoteIdentifier($table->table);

        $out = [];
        foreach ($table->renamedColumns() as $column) {
            $out[] = 'ALTER TABLE ' . $tableSql
                . ' RENAME COLUMN ' . $this->quoteIdentifier($column->from)
                . ' TO ' . $this->quoteIdentifier($column->to);
        }
    ...
    public function compileAlterTableAddForeignKeys(TableBlueprint $table): array
    {
        ...
        $out      = [];
        foreach ($keys as $fk) {
            $out[] = 'ALTER TABLE ' . $tableSql . ' ADD ' . $this->compileForeignKey($table, $fk);
        }
    ...
    public function compileAlterTableDropForeignKeys(TableBlueprint $table): array
    {
        ...
        foreach ($table->droppedForeignKeys() as $foreignKey) {
            $out[] = 'ALTER TABLE ' . $tableSql . ' DROP CONSTRAINT ' . $this->quoteIdentifier($foreignKey);
        }
    ...
    public function compileDropIndexes(TableBlueprint $table): array
    {
        ...
        foreach ($table->droppedIndexes() as $index) {
            $out[] = 'DROP INDEX IF EXISTS ' . $this->quoteIdentifier($index);
        }
    ...
    final protected function compileCreateIndex(string $table, IndexBlueprint $idx): string
    {
        ...
        $method = $idx->method !== null ? strtolower(trim($idx->method)) : null;
        if ($method !== null && !$this->supportsIndexMethod($method)) {
            throw new ConfigurationException('Index method "' . $method . '" is not supported by this schema compiler.');
        }

        $colsSql = [];
        foreach ($columns as $col) {
            $colsSql[] = $idx->expression ? '(' . $col . ')' : $this->quoteIdentifier($col);
        }

        $methodSql = $method !== null ? ' USING ' . strtoupper($method) : '';

        return sprintf(
            'CREATE %sINDEX%s %s ON %s%s (%s)',
            $idx->unique ? 'UNIQUE ' : '',
            $this->supportsCreateIndexIfNotExists() ? ' IF NOT EXISTS' : '',
    ...
    abstract protected function compileColumnDefinition(ColumnBlueprint $col, TableBlueprint $table): string;

    final protected function validateColumnCharacterOptions(ColumnBlueprint $col): void
    {
        if ($col->charset === null && $col->collation === null) {
            return;
        }

        // Свойства blueprint публичные: fluent-валидацию можно обойти.
        foreach (['charset' => $col->charset, 'collation' => $col->collation] as $option => $value) {
            if ($value !== null) {
                CharsetCollationName::normalize($value, $option);
            }
        }

        if (!$this->supportsColumnCharacterOptions()) {
            throw new ConfigurationException(sprintf(
                'Column charset/collation for "%s" are supported only by mysql/mariadb schema compilers.',
                $col->name,
            ));
        }

        if (!in_array($col->type, ['string', 'text'], true)) {
            throw new ConfigurationException(sprintf(
                'Column charset/collation are not supported for type "%s" (column "%s"); use string or text.',
                $col->type,
                $col->name,
            ));
        }
    ...
    final protected function hasDefault(ColumnBlueprint $col): bool
    {
        // $default остаётся публичным для обратной совместимости с blueprint-кодом,
        // который присваивает ненулевое значение напрямую.
        return $col->hasDefault || $col->default !== null;
    }

    final protected function validateGeneratedColumn(ColumnBlueprint $col): void
    {
        if ($col->generatedExpression === null) {
            return;
        }

        if (trim($col->generatedExpression) === '') {
            throw new ConfigurationException('Generated column expression must be non-empty.');
        }

        if ($this->hasDefault($col)) {
            throw new ConfigurationException('Generated columns cannot have a default value.');
        }

        if ($col->type === 'id' || $col->autoIncrement) {
            throw new ConfigurationException('Generated columns cannot use auto increment.');
        }
