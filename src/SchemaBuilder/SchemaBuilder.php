...
final readonly class SchemaBuilder implements SchemaBuilderInterface
{
    ...
    public function alterTable(string $table, callable $definition): void
    {
        ...
        foreach ($this->compiler->compileAlterTableDropForeignKeys($blueprint) as $sql) {
            $this->connection->execute($sql);
        }

        foreach ($this->compiler->compileDropIndexes($blueprint) as $sql) {
            $this->connection->execute($sql);
        }

        foreach ($this->compiler->compileAlterTableDropColumns($blueprint) as $sql) {
            $this->connection->execute($sql);
        }

        foreach ($this->compiler->compileAlterTableAddColumns($blueprint) as $sql) {
            $this->connection->execute($sql);
        }
        ...
        foreach ($this->compiler->compileAlterTableAddForeignKeys($blueprint) as $sql) {
            $this->connection->execute($sql);
        }
    }
