...
trait HavingAwareTrait
{
     ...
     *
     * @var list<array{boolean: 'AND'|'OR', type: 'condition'|'raw_condition', sql: string, params: array<string|int, mixed>}|array{boolean: 'AND'|'OR', type: 'group', nodes: array}>
     */
     ...
     *
     * ```
     * $conn->query()->select(['client_id'])->selectRaw('COUNT(*) as cnt')->from('orders')
     *     ->groupBy('client_id')
    ...
    public function having(string|callable $sql, array $params = []): static
    {
        ...
        $this->assertSimpleHavingString($sql);
    ...
    /**
     * Явно сырой HAVING-фрагмент (raw SQL).
     *
     * @param array<string|int, mixed> $params
     */
    public function havingRaw(string|Expression $sql, array $params = []): static
    {
    ...
    /**
     * OR-версия для raw HAVING-фрагмента.
     *
     * @param array<string|int, mixed> $params
     */
    public function orHavingRaw(string|Expression $sql, array $params = []): static
    {
        $value = trim((string) $sql);
        if ($value === '') {
            return $this;
    ...
    private function assertSimpleHavingString(string $sql): void
    {
        if ($this->isComplexHavingString($sql)) {
            throw new InvalidArgumentException(
                'Complex SQL is not allowed in having()/orHaving(). Use havingRaw()/orHavingRaw().',
            );
        }
    }

    private function isComplexHavingString(string $sql): bool
    {
