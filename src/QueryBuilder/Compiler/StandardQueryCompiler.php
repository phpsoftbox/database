...
final class StandardQueryCompiler extends AbstractQueryCompiler implements QueryCompilerInterface
{
    public function compileSelect(SelectQueryBuilder $builder): CompiledQuery
    {
        ...
        $lockClause = $builder->lockClause();
        if ($unions === [] && $lockClause !== null && $lockClause !== '') {
            $sql .= ' ' . $lockClause;
        }
     ...
     */
    private function compileSelectUnionWrapped(SelectQueryBuilder $builder): CompiledQuery
    {
        $base = $this->compileSelect($builder->resetPaginationAndOrderForUnion());

        $sql    = 'SELECT * FROM (' . $base->sql . ') AS _u';
        $params = $base->bindings;
    ...
    public function compileUpdate(UpdateQueryBuilder $builder): CompiledQuery
    {
        ...
        foreach ($cols as $col) {
            $value     = $data[$col];
            $quotedCol = $this->quoter->ident($col);

            if ($value instanceof Expression) {
                $setParts[] = $quotedCol . ' = ' . $value->sql;
