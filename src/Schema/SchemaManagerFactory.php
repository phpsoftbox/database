...
{
    public function create(ConnectionInterface $connection): SchemaManagerInterface
    {
        $driver = strtolower($connection->driver()->name());

        return match ($driver) {
            'sqlite'            => new SqliteSchemaManager($connection),
            'mysql'             => new MySqlSchemaManager($connection),
            'mariadb'           => new MariaDbSchemaManager($connection),
