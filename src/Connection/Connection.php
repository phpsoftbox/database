...
final class Connection implements WarmupAwareConnectionInterface
{
    private int $transactionLevel = 0;

    private WarmupStore $warmupStore;

    private ?WarmupLookup $warmupLookup = null;
    ...
    public function __construct(
        ...
        private readonly string $warmupConnectionName = 'default',
        ?WarmupStore $warmupStore = null,
        private readonly ?ProfilerInterface $profiler = null,
        private readonly ?DatabaseProfilerCollector $profilerCollector = null,
    ) {
        $this->warmupStore = $warmupStore ?? new WarmupStore();
    }
    ...
    {
        return $this->warmupLookup ??= new WarmupLookup(
            $this,
            $this->warmupStore,
            $this->warmupConnectionName,
        );
    }
    ...
    public function execute(string $sql, array $params = []): int
    {
        ...
        $executed = $this->prepareAndExecuteStatement($sql, $params);
        $rows     = $executed['stmt']->rowCount();

        $this->recordProfilerQuery(
            $executed['span'],
            $executed['sql'],
            $executed['bindings'],
            $executed['elapsed_ms'],
     ...
     * }
     */
    private function prepareAndExecuteStatement(string $sql, array $params): array
    {
        $start = hrtime(true);
        $span  = $this->startProfilerQuery($sql, $params);
     ...
     * @param array<string|int, mixed> $params
     */
    private function startProfilerQuery(string $sql, array $params): ?SpanInterface
    {
        if ($this->profiler === null || !$this->profiler->enabled()) {
            return null;
        }

        return $this->profiler->start('db.query', [
            'connection'   => $this->warmupConnectionName,
     ...
     */
    private function recordProfilerQuery(
        ?SpanInterface $span,
        string $sql,
        array $bindings,
        float $elapsedMs,
        ...
        $span?->addTag('row_count', $rowCount);
        $span?->finish();
