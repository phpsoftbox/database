...
final readonly class Database
{
    ...
    /**
     * Экранирует одиночный идентификатор под драйвер указанного подключения.
     */
    public function quoteIdentifier(string $identifier, string $connection = 'default'): string
    {
        return $this->connection($connection)->quoteIdentifier($identifier);
    }

    /**
     * Возвращает имя таблицы с префиксом и экранирует его под драйвер указанного подключения.
     */
    public function quoteTable(string $table, string $connection = 'default'): string
    {
        return $this->connection($connection)->quoteTable($table);
    }
    ...
    public function schema(string $connection = 'default'): SchemaManagerInterface
    {
        ...
        return new SchemaManagerFactory()->create($conn);
    }
}
