...
### Транзакции и race conditions: нужно ли всегда делать lock?

Общее правило: **транзакция сама по себе не лечит race condition**. Она лишь группирует операции.
Если внутри транзакции вы:
- прочитали строку (SELECT)
- приняли решение на основе прочитанного
- потом обновили (UPDATE)

то без дополнительной синхронизации две транзакции могут прочитать одно и то же состояние и обе
попытаются обновить, что и создаёт race condition.

Практические подходы (выбираются по ситуации):

1) **Pessimistic locking** (блокировки строк)
   - Postgres / MySQL: `SELECT ... FOR UPDATE` (или `FOR SHARE` для читателей)
   - Используется, когда "кто первый встал, того и тапки" и цена конфликта высока.

2) **Optimistic locking** (версионность)
   - Добавляете поле `version` или `updated_at`, и апдейт делаете так:
     `UPDATE ... SET ..., version = version + 1 WHERE id = :id AND version = :expected`
   - Если `rowCount()` = 0, значит кто-то обновил раньше — повторить/ошибка.
   - Обычно проще масштабировать и меньше блокировок.

3) **Атомарные UPDATE/INSERT** вместо read-then-write
   - Например, инкремент счётчика делаем одной командой:
     `UPDATE counters SET value = value + 1 WHERE id = :id`
   - Или используем UPSERT (`INSERT ... ON CONFLICT DO UPDATE` в Postgres).

4) **Изоляция транзакций**
   - Повышение уровня изоляции может помочь, но часто ведёт к деградации производительности и росту блокировок.
   - Не стоит пытаться включить "самый строгий" уровень везде автоматически.

Почему "автоматически всегда делать lock" — плохая идея:
- разные кейсы требуют разных стратегий (где-то lock нужен, где-то убьёт throughput)
- lock может приводить к дедлокам
- иногда правильнее поменять SQL на атомарный

Рекомендация для DBAL на текущем этапе:
- оставить `transaction()` как механизм группировки
- в бизнес-кейсах явно решать стратегию: `SELECT ... FOR UPDATE`, optimistic locking, атомарный SQL
- позже мы можем добавить удобные helper-методы, но не делать скрытую магию
