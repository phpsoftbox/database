...
final class QueryBuilderTest extends TestCase
{
    ...
    /**
     * Повтор одного и того же named placeholder в разных where-фрагментах
     * не должен приводить к конфликтам биндинга.
     */
    #[Test]
    public function repeatedNamedPlaceholderAcrossWhereFragmentsIsRewritten(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        $built = $conn->query()
            ->select()
            ->from('tickets t')
            ->where('t.subject LIKE :query', ['query' => '%urgent%'])
            ->orWhere('t.description LIKE :query', ['query' => '%urgent%'])
            ->orWhere('t.note LIKE :query', ['query' => '%urgent%'])
            ->toSql();

        self::assertSame(
            'SELECT * FROM "tickets" AS "t" WHERE ("t"."subject" LIKE :query) OR ("t"."description" LIKE :__qb_auto_1) OR ("t"."note" LIKE :__qb_auto_2)',
            $built['sql'],
        );
        self::assertSame(
            [
    ...
    /**
     * Проверяет, что whereRaw() не прогоняет SQL-функции через эвристический quoting.
     */
    #[Test]
    public function whereRawKeepsSqlExpressionUntouched(): void
    {
        $conn = new SpyConnection(new FakePdo('pgsql'));

        $built = $conn->query()
            ->select()
            ->from('products')
            ->whereRaw("search_vector @@ websearch_to_tsquery('russian', :query)", ['query' => 'телефон'])
            ->toSql();

        self::assertSame(
            'SELECT * FROM "products" WHERE (search_vector @@ websearch_to_tsquery(\'russian\', :query))',
            $built['sql'],
        );
        self::assertSame(['query' => 'телефон'], $built['params']);
    }

    /**
     * Проверяет, что повтор named placeholder в raw WHERE-фрагментах переписывается без quoting SQL.
     */
    #[Test]
    public function repeatedNamedPlaceholderAcrossRawWhereFragmentsIsRewritten(): void
    {
        $conn = new SpyConnection(new FakePdo('pgsql'));
        ...
        $built = $conn->query()
            ->select()
            ->from('products')
            ->whereRaw('natural_search_vector @@ websearch_to_tsquery(:query)', ['query' => 'телефон'])
            ->orWhereRaw('technical_search_vector @@ websearch_to_tsquery(:query)', ['query' => 'телефон'])
            ->toSql();

        self::assertSame(
            'SELECT * FROM "products" WHERE (natural_search_vector @@ websearch_to_tsquery(:query)) OR (technical_search_vector @@ websearch_to_tsquery(:__qb_auto_1))',
            $built['sql'],
        );
        self::assertSame(
            [
    ...
    /**
     * Проверяет PostgreSQL full-text WHERE через tsvector @@ tsquery.
     */
    #[Test]
    public function wherePgFullTextBuildsPostgresTsQueryCondition(): void
    {
        ...
        self::assertSame(
            'SELECT * FROM "products" AS "p" WHERE ("p"."search_vector" @@ websearch_to_tsquery(\'russian\', :__pg_fts_query_1))',
            $built['sql'],
        );
        self::assertSame(['__pg_fts_query_1' => 'телефон'], $built['params']);
    }

    /**
     * Проверяет PostgreSQL full-text rank как computed SELECT column.
     */
    #[Test]
    public function selectPgFullTextRankBuildsComputedColumn(): void
    {
        $conn = new SpyConnection(new FakePdo('pgsql'), driver: new PostgresDriver());
        ...
        $built = $conn->query()
            ->select('p.*')
            ->from('products p')
            ->wherePgFullText('p.search_vector', 'телефон', new PgFullTextOptions(config: 'russian'))
            ->selectPgFullTextRank(
                'p.search_vector',
                'телефон',
                'search_rank',
                new PgFullTextOptions(
                    config: 'russian',
        ...
        self::assertSame(
            'SELECT "p".*, ts_rank("p"."search_vector", plainto_tsquery(\'russian\', :__pg_fts_query_2), 32) AS "search_rank" FROM "products" AS "p" WHERE ("p"."search_vector" @@ websearch_to_tsquery(\'russian\', :__pg_fts_query_1)) ORDER BY "search_rank" DESC',
            $built['sql'],
        );
        self::assertSame(
            [
    ...
    /**
     * Проверяет PostgreSQL ts_headline как computed SELECT column.
     */
    #[Test]
    public function selectPgFullTextHeadlineBuildsComputedColumn(): void
    {
        $conn = new SpyConnection(new FakePdo('pgsql'), driver: new PostgresDriver());
        ...
        $built = $conn->query()
            ...
            ->selectPgFullTextHeadline(
                ...
                'телефон',
                'search_headline',
                new PgFullTextOptions(config: 'russian'),
                'StartSel=<mark>, StopSel=</mark>',
            )
            ->toSql();

        self::assertSame(
            'SELECT "p".*, ts_headline(\'russian\', "p"."description", websearch_to_tsquery(\'russian\', :__pg_fts_query_1), \'StartSel=<mark>, StopSel=</mark>\') AS "search_headline" FROM "products" AS "p"',
            $built['sql'],
        );
        self::assertSame(['__pg_fts_query_1' => 'телефон'], $built['params']);
    }
    ...
    /**
     * Проверяет, что пустой поисковый запрос по умолчанию не добавляет WHERE.
     */
    #[Test]
    public function wherePgFullTextSkipsEmptyQueryByDefault(): void
    {
        $conn = new SpyConnection(new FakePdo('pgsql'), driver: new PostgresDriver());
        ...
        $built = $conn->query()
            ->select()
            ->from('products')
            ->wherePgFullText('search_vector', '   ')
            ->toSql();

        self::assertSame('SELECT * FROM "products"', $built['sql']);
        self::assertSame([], $built['params']);
    }

    /**
     * Проверяет, что PostgreSQL-specific query API не генерирует SQL для других драйверов.
     */
    #[Test]
    public function wherePgFullTextRequiresPostgresDriver(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $this->expectException(ConfigurationException::class);
        $this->expectExceptionMessage('supported only by postgres driver');

        $conn->query()
            ->select()
            ->from('products')
            ->wherePgFullText('search_vector', 'телефон');
    }

    /**
     * Повтор placeholder внутри одного SQL-фрагмента должен разворачиваться
     * в уникальные параметры, чтобы исключить HY093.
     */
    #[Test]
    public function repeatedNamedPlaceholderInsideSingleFragmentIsRewritten(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        $built = $conn->query()
            ->select()
            ->from('users u')
            ->whereRaw('(u.name LIKE :query OR u.email LIKE :query)', ['query' => '%john%'])
            ->toSql();

        self::assertStringContainsString('WHERE ((u.name LIKE :__qb_auto_1 OR', $built['sql']);
        self::assertStringContainsString('u.email LIKE :__qb_auto_2))', $built['sql']);
        self::assertSame(
            [
                '__qb_auto_1' => '%john%',
                '__qb_auto_2' => '%john%',
    ...
    #[Test]
    public function updateSupportsRawExpressionsAndKeepsOtherValuesBound(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'), prefix: 't_');
        ...
        self::assertSame(
            'UPDATE "t_warehouse_places" SET "max_load" = max_load * 1000, "updated_by" = :v_1 WHERE ("id" = :id)',
            $built['sql'],
        );
        self::assertSame(['v_1' => 42, 'id' => 5], $built['params']);

        $builder->execute();

        self::assertSame($built['sql'], $conn->executed[0]['sql']);
        self::assertSame($built['params'], $conn->executed[0]['params']);
    }
    ...
    /**
     * Проверяет havingRaw/orHavingRaw и merge параметров с WHERE.
     */
    #[Test]
    ...
    /**
     * Проверяет структурный DSL where(array): key=>value, list-условия и IN shorthand.
     */
    #[Test]
    public function whereArrayDslBuildsSqlAndMergesParams(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        $built = $conn->query()
            ...
            ->where([
                ...
                'created_to'   => '2026-01-31 23:59:59',
            ])
            ->toSql();

        self::assertSame(
            'SELECT "u".* FROM "users" AS "u" WHERE ("u"."status" = :status) AND ("u"."created_datetime" >= :created_from) AND ("u"."created_datetime" <= :created_to) AND ("u"."id" IN (:in_1, :in_2, :in_3))',
            $built['sql'],
        );
        self::assertSame(
            [
    ...
    /**
     * Проверяет поддержку формата column/operator/target_column в where(array).
     */
    #[Test]
    public function whereArraySupportsTargetColumnComparison(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        self::assertSame(
            'SELECT * FROM "users" AS "u" WHERE ("u"."owner_id" = "o"."id")',
            $built['sql'],
        );
        self::assertSame([], $built['params']);
    }
    ...
    /**
     * Проверяет whereRaw()/orWhereRaw() для сложных выражений.
     */
    #[Test]
    public function whereRawBuildsSqlAndMergesParams(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        $built = $conn->query()
            ->select()
            ->from('users u')
            ->whereRaw('COALESCE(u.last_total_bytes, 0) > :min_total', ['min_total' => 0])
            ->orWhereRaw('u.last_reported_datetime IS NOT NULL')
            ->toSql();

        self::assertSame(
            'SELECT * FROM "users" AS "u" WHERE (COALESCE(u.last_total_bytes, 0) > :min_total) OR (u.last_reported_datetime IS NOT NULL)',
            $built['sql'],
        );
        self::assertSame(['min_total' => 0], $built['params']);
    }

    /**
     * Проверяет havingRaw()/orHavingRaw().
     */
    #[Test]
    public function havingRawBuildsSqlAndMergesParams(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        self::assertSame(
            'SELECT "client_id", COUNT(*) AS "cnt" FROM "users" GROUP BY "client_id" HAVING (COUNT(*) > :min) OR (SUM(total) > :sum)',
            $built['sql'],
        );
        self::assertSame(['min' => 10, 'sum' => 1000], $built['params']);
    }
    ...
    public function orHavingCallableBuildsGroupedSql(): void
    {
        ...
        $built = $conn->query()
            ...
            ->orHaving(function (SelectQueryBuilder $q): void {
                $q
                    ->havingRaw('COUNT(*) < :max', ['max' => 100])
                    ->havingRaw('COUNT(*) != :neq', ['neq' => 50]);
            })
            ->toSql();
    ...
    /**
     * Проверяет, что сложные выражения в where() запрещены и требуют whereRaw().
     */
    #[Test]
    public function whereRejectsComplexSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Complex SQL is not allowed in where()/orWhere()');
    ...
    /**
     * Проверяет, что сложные выражения в having() запрещены и требуют havingRaw().
     */
    #[Test]
    public function havingRejectsComplexSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Complex SQL is not allowed in having()/orHaving()');
    ...
    /**
     * Проверяет, что сложные выражения в select() запрещены и требуют selectRaw()/Expression.
     */
    #[Test]
    public function selectRejectsComplexSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Complex SQL is not allowed in select()');
    ...
    /**
     * Проверяет selectRaw() для сложных выражений в SELECT.
     */
    #[Test]
    public function selectRawBuildsSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        self::assertSame(
            'SELECT "u"."id", COUNT(*) AS "total" FROM "users" AS "u" GROUP BY "u"."id"',
            $built['sql'],
        );
    }

    /**
     * Проверяет, что selectRaw() принимает параметры для вычисляемых SELECT-выражений.
     */
    #[Test]
    public function selectRawAcceptsParams(): void
    {
        $conn = new SpyConnection(new FakePdo('pgsql'));
        ...
        $built = $conn->query()
            ->select('p.*')
            ->selectRaw(
                'ts_rank_cd(search_vector, websearch_to_tsquery(:query)) AS search_rank',
                ['query' => 'телефон'],
            )
            ->from('products p')
            ->toSql();
        ...
        self::assertSame(
            'SELECT "p".*, ts_rank_cd(search_vector, websearch_to_tsquery(:query)) AS "search_rank" FROM "products" AS "p"',
            $built['sql'],
        );
        self::assertSame(['query' => 'телефон'], $built['params']);
    }
    ...
    /**
     * Проверяет, что count() не наследует параметры вычисляемых SELECT-выражений.
     */
    #[Test]
    public function countDropsSelectRawParamsWhenReplacingSelectColumns(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        $conn->query()
            ->select('p.*')
            ->selectRaw(
                '(SELECT SUM(quantity) FROM stock_balance_positions WHERE balance_type = :balance_type) AS balance_available',
        ...
        self::assertSame(
            'SELECT COUNT(*) AS __agg FROM "products" AS "p"',
            $conn->executed[0]['sql'] ?? null,
        );
        self::assertSame([], $conn->executed[0]['params'] ?? null);
    }

    /**
     * Проверяет, что exists() не наследует параметры вычисляемых SELECT-выражений.
     */
    #[Test]
    public function existsDropsSelectRawParamsWhenReplacingSelectColumns(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        self::assertSame(
            'SELECT 1 FROM "products" AS "p" LIMIT 1',
            $conn->executed[0]['sql'] ?? null,
        );
        self::assertSame([], $conn->executed[0]['params'] ?? null);
    }
    ...
    /**
     * Проверяет helper-ы для агрегатных SELECT-выражений рядом со строкой результата.
     */
    #[Test]
    public function selectAggregateHelpersBuildComputedColumns(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $built = $conn->query()
            ->select('p.id')
            ->selectCount('oi.id', 'items_count')
            ->selectSum('oi.quantity', 'items_quantity')
            ->from('products p')
        ...
        self::assertSame(
            'SELECT "p"."id", COUNT("oi"."id") AS "items_count", SUM("oi"."quantity") AS "items_quantity" FROM "products" AS "p" LEFT JOIN order_items oi ON "oi"."product_id" = "p"."id" GROUP BY "p"."id"',
            $built['sql'],
        );
        self::assertSame([], $built['params']);
    }
    ...
    /**
     * Проверяет, что lockForUpdate() добавляет FOR UPDATE в конец SELECT.
     */
    #[Test]
    public function lockForUpdateBuildsSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        self::assertSame('SELECT * FROM "users" WHERE ("id" = :id) FOR UPDATE', $built['sql']);
        self::assertSame(['id' => 10], $built['params']);
    }

    /**
     * Проверяет shared-lock режимы: FOR SHARE и LOCK IN SHARE MODE.
     */
    #[Test]
    public function sharedLockModesBuildSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        self::assertSame('SELECT * FROM "users" FOR SHARE', $forShare['sql']);

        $inShareMode = $conn->query()
            ->select()
            ->from('users')
            ->lockInShareMode()
            ->toSql();

        self::assertSame('SELECT * FROM "users" LOCK IN SHARE MODE', $inShareMode['sql']);
    }

    /**
     * Проверяет универсальный lock() и суффиксы NOWAIT/SKIP LOCKED.
     */
    #[Test]
    public function genericLockAndLockSuffixesBuildSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
        ...
        $explicit = $conn->query()
            ...
            ->toSql();
        self::assertSame('SELECT * FROM "jobs" FOR UPDATE SKIP LOCKED', $explicit['sql']);
        ...
        $nowait = $conn->query()
            ...
            ->lockNoWait()
            ->toSql();
        self::assertSame('SELECT * FROM "jobs" FOR UPDATE NOWAIT', $nowait['sql']);

    }

    /**
     * Проверяет MariaDB-специфичный суффикс WAIT n.
     */
    #[Test]
    public function mariaDbLockWaitBuildsSql(): void
    {
        ...
        $wait = $conn->query()
            ...
            ->lockWait(3)
            ->toSql();
        self::assertSame('SELECT * FROM `jobs` FOR SHARE WAIT 3', $wait['sql']);
    }

    /**
     * Проверяет, что MySQL-драйвер не пропускает MariaDB-специфичный WAIT n.
     */
    #[Test]
    public function mySqlRejectsMariaDbLockWait(): void
    {
        $conn = new SpyConnection(new FakePdo('mysql'), driver: new MySqlDriver());

        $this->expectException(ConfigurationException::class);
        $this->expectExceptionMessage('lockWait() is supported only by the MariaDB driver.');
    ...
    /**
     * Проверяет PostgreSQL режимы FOR NO KEY UPDATE / FOR KEY SHARE.
     */
    #[Test]
    public function postgresLockModesBuildSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $noKeyUpdate = $conn->query()
            ->select()
            ->from('users')
            ->lockForNoKeyUpdate()
            ->toSql();
        self::assertSame('SELECT * FROM "users" FOR NO KEY UPDATE', $noKeyUpdate['sql']);
        ...
        $keyShare = $conn->query()
            ...
            ->lockForKeyShare()
            ->toSql();
        self::assertSame('SELECT * FROM "users" FOR KEY SHARE', $keyShare['sql']);
    }
    ...
    public function leftJoinSubqueryBuildsSql(): void
    {
        ...
        $built = $conn->query()
            ...
            ->leftJoinSubquery(function (SelectQueryBuilder $q): void {
                $q->select(['user_id'])->selectRaw('COUNT(*) AS cnt')->from('orders')->groupBy('user_id');
            }, 'o', 'o.user_id = u.id')
            ->toSql();
    ...
    public function joinSubqueryWrapsInParentheses(): void
    {
        ...
        $sub = $conn->query()->select(['user_id'])->selectRaw('COUNT(*) AS cnt')->from('orders')->groupBy('user_id');
    ...
    /**
     * Проверяет, что exists() собирает оптимизированный SQL (SELECT 1 ... LIMIT 1)
     * и возвращает true при наличии строки.
     */
    #[Test]
    public function existsBuildsOptimizedSqlAndReturnsTrueWhenRowFound(): void
    {
        $conn = new class (new FakePdo('sqlite')) extends SpyConnection {
            public function fetchOne(string $sql, array $params = []): ?array
            {
        ...
        self::assertCount(1, $conn->executed);
        self::assertSame('SELECT 1 FROM "users" WHERE ("active" = :active) LIMIT 1', $conn->executed[0]['sql']);
        self::assertSame(['active' => 1], $conn->executed[0]['params']);
    }

    /**
     * Проверяет, что exists() возвращает false, когда запрос не находит ни одной строки.
     */
    #[Test]
    public function existsReturnsFalseWhenNoRowsFound(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));
    ...
    /**
     * Проверяет, что notExists() использует тот же оптимизированный SQL и возвращает true при отсутствии строк.
     */
    #[Test]
    public function notExistsBuildsOptimizedSqlAndReturnsTrueWhenNoRowsFound(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $notExists = $conn->query()
            ->select()
            ->from('users')
            ->where('id = :id', ['id' => 999])
            ->orderBy('id', 'DESC')
        ...
        self::assertTrue($notExists);
        self::assertCount(1, $conn->executed);
        self::assertSame('SELECT 1 FROM "users" WHERE ("id" = :id) LIMIT 1', $conn->executed[0]['sql']);
        self::assertSame(['id' => 999], $conn->executed[0]['params']);
    }
