...
final class PostgresSchemaManagerTest extends TestCase
{
    ...
    public function introspectsSchemaInPostgres(): void
    {
        ...
        self::assertTrue($schema->hasIndex('users', 'users_email_uq'));
        self::assertFalse($schema->hasIndex('users', 'missing_idx'));

        $fks = $schema->foreignKeys('orders');
        self::assertNotEmpty($fks);
        self::assertSame('orders_user_fk', $fks[0]->name);
        self::assertTrue($schema->hasForeignKey('orders', 'orders_user_fk'));
        self::assertFalse($schema->hasForeignKey('orders', 'missing_fk'));
        self::assertNotNull($schema->foreignKey('orders', 'orders_user_fk'));
        ...
        $missing = $schema->missingColumns('users', ['id', 'email', 'missing_email']);
        self::assertSame(['missing_email'], $missing->all());
        self::assertTrue($missing->has('missing_email'));
        self::assertFalse($missing->has('email'));
