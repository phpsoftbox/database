...
final class TableBlueprintModifiersTest extends TestCase
{
    ...
    /**
     * Проверяет разделение новых и изменяемых колонок.
     */
    #[Test]
    public function separatesAddedAndChangedColumns(): void
    {
        $t = new TableBlueprint('users');

        $email = $t->string('email', 320)->change();
        $phone = $t->string('phone', 32);

        self::assertSame([$phone], $t->addedColumns());
        self::assertSame([$email], $t->changedColumns());
    }

    /**
     * Проверяет, что renameColumn() сохраняет операции переименования колонок.
     */
    #[Test]
    public function storesRenameColumnOperations(): void
    {
        $t = new TableBlueprint('users');

        $t->renameColumn(' display_name ', ' name ');

        self::assertCount(1, $t->renamedColumns());
        self::assertSame('display_name', $t->renamedColumns()[0]->from);
        self::assertSame('name', $t->renamedColumns()[0]->to);
    }
