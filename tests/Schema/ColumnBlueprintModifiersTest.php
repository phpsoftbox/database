...
final class ColumnBlueprintModifiersTest extends TestCase
{
    ...
    /**
     * Проверяет флаг изменения существующей колонки.
     */
    #[Test]
    public function storesChangeModifier(): void
    {
        $column = new ColumnBlueprint('email', 'string');

        $column->change();

        self::assertTrue($column->change);

        $column->change(false);

        self::assertFalse($column->change);
    }
