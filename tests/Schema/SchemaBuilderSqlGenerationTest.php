<?php

declare(strict_types=1);

namespace PhpSoftBox\Database\Tests\Schema;

use PhpSoftBox\Database\Exception\ConfigurationException;
use PhpSoftBox\Database\Postgres\FullText\PgSearchVectorExpression;
use PhpSoftBox\Database\SchemaBuilder\ColumnRenameBlueprint;
use PhpSoftBox\Database\SchemaBuilder\Compiler\MariaDbSchemaCompiler;
use PhpSoftBox\Database\SchemaBuilder\Compiler\PostgresSchemaCompiler;
use PhpSoftBox\Database\SchemaBuilder\Compiler\SqliteSchemaCompiler;
use PhpSoftBox\Database\SchemaBuilder\SchemaBuilder;
use PhpSoftBox\Database\SchemaBuilder\TableBlueprint;
use PhpSoftBox\Database\SchemaBuilder\UseCurrentFormatsEnum;
use PhpSoftBox\Database\Tests\Utils\FakePdo;
use PhpSoftBox\Database\Tests\Utils\SpyConnection;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\Test;
use PHPUnit\Framework\TestCase;

#[CoversClass(SchemaBuilder::class)]
#[CoversClass(ColumnRenameBlueprint::class)]
#[CoversClass(SqliteSchemaCompiler::class)]
#[CoversClass(MariaDbSchemaCompiler::class)]
#[CoversClass(PostgresSchemaCompiler::class)]
#[CoversClass(PgSearchVectorExpression::class)]
final class SchemaBuilderSqlGenerationTest extends TestCase
{
    /**
     * Проверяет, что SchemaBuilder::create() выполняет SQL, сгенерированный компилятором,
     * и по умолчанию использует IF NOT EXISTS.
     */
    #[Test]
    public function createExecutesGeneratedSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->create('users', function (TableBlueprint $t): void {
            $t->id();
            $t->text('name')->nullable(true);
        });

        self::assertCount(1, $conn->executed);
        self::assertSame('CREATE TABLE IF NOT EXISTS "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT)', $conn->executed[0]['sql']);
    }

    /**
     * Проверяет, что SchemaBuilder::create() может быть вызван без IF NOT EXISTS.
     */
    #[Test]
    public function createWithoutIfNotExistsExecutesGeneratedSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->create('users', function (TableBlueprint $t): void {
            $t->id();
            $t->text('name');
        }, false);

        self::assertCount(1, $conn->executed);
        self::assertSame('CREATE TABLE "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT NOT NULL)', $conn->executed[0]['sql']);
    }

    /**
     * Проверяет, что SchemaBuilder::alterTable() выполняет SQL для добавления колонок.
     */
    #[Test]
    public function alterTableExecutesAddColumnSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->alterTable('users', function (TableBlueprint $t): void {
            $t->text('email')->nullable(true);
        });

        self::assertCount(1, $conn->executed);
        self::assertSame('ALTER TABLE "users" ADD COLUMN "email" TEXT', $conn->executed[0]['sql']);
    }

    /**
     * Проверяет, что change() не попадает в ADD COLUMN.
     */
    #[Test]
    public function alterTableDoesNotAddChangedColumn(): void
    {
        $conn = new SpyConnection(new FakePdo('mysql'));

        $builder = new SchemaBuilder($conn, new MariaDbSchemaCompiler());

        $builder->alterTable('users', function (TableBlueprint $t): void {
            $t->string('email', 320)->nullable()->change();
            $t->string('phone', 32)->nullable();
        });

        self::assertCount(2, $conn->executed);
        self::assertSame('ALTER TABLE `users` ADD COLUMN `phone` VARCHAR(32)', $conn->executed[0]['sql']);
        self::assertSame('ALTER TABLE `users` MODIFY COLUMN `email` VARCHAR(320)', $conn->executed[1]['sql']);
    }

    /**
     * Проверяет, что SQLite явно отказывается от change() до выполнения SQL.
     */
    #[Test]
    public function sqliteChangeColumnThrowsBeforeExecutingSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $this->expectException(ConfigurationException::class);
        $this->expectExceptionMessage('SQLite does not support changing columns through ALTER TABLE.');

        try {
            $builder->alterTable('users', function (TableBlueprint $t): void {
                $t->string('phone', 32)->nullable();
                $t->string('email', 320)->nullable()->change();
            });
        } finally {
            self::assertSame([], $conn->executed);
        }
    }

    /**
     * Проверяет, что SchemaBuilder::addColumn() является обёрткой над alterTable().
     */
    #[Test]
    public function addColumnExecutesAlterTable(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->addColumn('users', function (TableBlueprint $t): void {
            $t->integer('age')->nullable();
        });

        self::assertCount(1, $conn->executed);
        self::assertSame('ALTER TABLE "users" ADD COLUMN "age" INTEGER', $conn->executed[0]['sql']);
    }

    /**
     * Проверяет, что SchemaBuilder::alterTable() поддерживает добавление только индекса
     * без операций добавления колонок.
     */
    #[Test]
    public function alterTableExecutesIndexSqlWithoutColumns(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->alterTable('users', function (TableBlueprint $t): void {
            $t->index([
                'email',
            ], 'users_email_idx');
        });

        self::assertCount(1, $conn->executed);
        self::assertSame(
            'CREATE INDEX IF NOT EXISTS "users_email_idx" ON "users" ("email")',
            $conn->executed[0]['sql'],
        );
    }

    /**
     * Проверяет, что SchemaBuilder::alterTable() выполняет dropIndex/dropColumn.
     */
    #[Test]
    public function alterTableExecutesDropIndexAndDropColumnSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->alterTable('users', function (TableBlueprint $t): void {
            $t->dropIndex('users_legacy_email_idx');
            $t->dropColumn('legacy_email');
        });

        self::assertCount(2, $conn->executed);
        self::assertSame(
            'DROP INDEX IF EXISTS "users_legacy_email_idx"',
            $conn->executed[0]['sql'],
        );
        self::assertSame(
            'ALTER TABLE "users" DROP COLUMN "legacy_email"',
            $conn->executed[1]['sql'],
        );
    }

    /**
     * Проверяет, что SchemaBuilder::alterTable() выполняет renameColumn() после dropIndex и до dropColumn.
     */
    #[Test]
    public function alterTableExecutesRenameColumnSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->alterTable('users', function (TableBlueprint $t): void {
            $t->dropIndex('users_display_name_idx');
            $t->renameColumn('display_name', 'name');
            $t->dropColumn('legacy_name');
        });

        self::assertCount(3, $conn->executed);
        self::assertSame(
            'DROP INDEX IF EXISTS "users_display_name_idx"',
            $conn->executed[0]['sql'],
        );
        self::assertSame(
            'ALTER TABLE "users" RENAME COLUMN "display_name" TO "name"',
            $conn->executed[1]['sql'],
        );
        self::assertSame(
            'ALTER TABLE "users" DROP COLUMN "legacy_name"',
            $conn->executed[2]['sql'],
        );
    }

    /**
     * Проверяет, что SchemaBuilder::alterTable() для MariaDB обрабатывает drop/add foreign key.
     */
    #[Test]
    public function alterTableExecutesDropAndAddForeignKeySqlForMariaDb(): void
    {
        $conn = new SpyConnection(new FakePdo('mysql'));

        $builder = new SchemaBuilder($conn, new MariaDbSchemaCompiler());

        $builder->alterTable('orders', function (TableBlueprint $t): void {
            $t->dropForeignKey('orders_user_fk');
            $t->dropColumn('user_id');
            $t->foreignId('user_id')->nullable();
            $t->foreignKey(['user_id'], 'users', ['id'], 'orders_user_fk')->onDelete('set null');
        });

        self::assertCount(4, $conn->executed);
        self::assertSame('ALTER TABLE `orders` DROP FOREIGN KEY `orders_user_fk`', $conn->executed[0]['sql']);
        self::assertSame('ALTER TABLE `orders` DROP COLUMN `user_id`', $conn->executed[1]['sql']);
        self::assertSame('ALTER TABLE `orders` ADD COLUMN `user_id` BIGINT UNSIGNED', $conn->executed[2]['sql']);
        self::assertSame(
            'ALTER TABLE `orders` ADD CONSTRAINT `orders_user_fk` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE SET NULL',
            $conn->executed[3]['sql'],
        );
    }

    /**
     * Проверяет, что SchemaBuilder::renameTable() выполняет корректный SQL.
     */
    #[Test]
    public function renameTableExecutesSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->renameTable('users', 'people');

        self::assertCount(1, $conn->executed);
        self::assertSame('ALTER TABLE "users" RENAME TO "people"', $conn->executed[0]['sql']);
    }

    /**
     * Проверяет, что SchemaBuilder::dropIfExists() выполняет корректный SQL.
     */
    #[Test]
    public function dropIfExistsExecutesSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->dropIfExists('users');

        self::assertCount(1, $conn->executed);
        self::assertSame('DROP TABLE IF EXISTS "users"', $conn->executed[0]['sql']);
    }

    /**
     * Проверяет, что SchemaBuilder::drop() выполняет корректный SQL.
     */
    #[Test]
    public function dropExecutesSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->drop('users');

        self::assertCount(1, $conn->executed);
        self::assertSame('DROP TABLE "users"', $conn->executed[0]['sql']);
    }

    /**
     * Проверяет генерацию SQL для таблицы MariaDB с модификаторами таблицы.
     */
    #[Test]
    public function mariaDbCreateTableIncludesTableModifiers(): void
    {
        $t = new TableBlueprint('users');

        $t->engine('InnoDB')->charset('utf8mb4')->collation('utf8mb4_unicode_ci')->comment('Users')->temporary(true);
        $t->id();

        $sql = new MariaDbSchemaCompiler()->compileCreateTable($t);

        self::assertStringStartsWith('CREATE TEMPORARY TABLE `users`', $sql);
        self::assertStringContainsString('ENGINE=InnoDB', $sql);
        self::assertStringContainsString('DEFAULT CHARSET=utf8mb4', $sql);
        self::assertStringContainsString('COLLATE=utf8mb4_unicode_ci', $sql);
        self::assertStringContainsString("COMMENT='Users'", $sql);
    }

    /**
     * Проверяет, что MariaDB создаёт колонки с COMMENT.
     */
    #[Test]
    public function mariaDbCreateTableIncludesColumnComments(): void
    {
        $t = new TableBlueprint('users');

        $t->id();
        $t->string('email', 255)->comment('Email address');

        $sql = new MariaDbSchemaCompiler()->compileCreateTable($t);

        self::assertStringContainsString("`email` VARCHAR(255) NOT NULL COMMENT 'Email address'", $sql);
    }

    /**
     * Проверяет, что MariaDB создаёт комментарии для всех типов колонок.
     */
    #[Test]
    public function mariaDbCreateTableIncludesCommentsForAllColumnTypes(): void
    {
        $t = new TableBlueprint('all_types');

        $t->id()->comment('ID');
        $t->integer('count')->comment('Count');
        $t->boolean('is_active')->comment('Active');
        $t->text('description')->comment('Description');
        $t->string('title', 100)->comment('Title');
        $t->json('payload')->comment('Payload');
        $t->foreignId('user_id')->comment('User');
        $t->datetime('created_datetime')->comment('Created');
        $t->date('publish_date')->comment('Publish date');
        $t->time('publish_time')->comment('Publish time');
        $t->timestamp('updated_at')->comment('Updated');

        $sql = new MariaDbSchemaCompiler()->compileCreateTable($t);

        self::assertStringContainsString("`id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'ID'", $sql);
        self::assertStringContainsString("`count` INT NOT NULL COMMENT 'Count'", $sql);
        self::assertStringContainsString("`is_active` TINYINT(1) NOT NULL COMMENT 'Active'", $sql);
        self::assertStringContainsString("`description` TEXT NOT NULL COMMENT 'Description'", $sql);
        self::assertStringContainsString("`title` VARCHAR(100) NOT NULL COMMENT 'Title'", $sql);
        self::assertStringContainsString("`payload` JSON NOT NULL COMMENT 'Payload'", $sql);
        self::assertStringContainsString("`user_id` BIGINT UNSIGNED NOT NULL COMMENT 'User'", $sql);
        self::assertStringContainsString("`created_datetime` DATETIME NOT NULL COMMENT 'Created'", $sql);
        self::assertStringContainsString("`publish_date` DATE NOT NULL COMMENT 'Publish date'", $sql);
        self::assertStringContainsString("`publish_time` TIME NOT NULL COMMENT 'Publish time'", $sql);
        self::assertStringContainsString("`updated_at` TIMESTAMP NOT NULL COMMENT 'Updated'", $sql);
    }

    /**
     * Проверяет, что MariaDB ALTER TABLE ADD COLUMN поддерживает FIRST/AFTER.
     */
    #[Test]
    public function mariaDbAlterTableSupportsFirstAfter(): void
    {
        $t = new TableBlueprint('users');

        $t->text('b')->after('a');

        $sqlList = new MariaDbSchemaCompiler()->compileAlterTableAddColumns($t);

        self::assertCount(1, $sqlList);
        self::assertSame('ALTER TABLE `users` ADD COLUMN `b` TEXT NOT NULL AFTER `a`', $sqlList[0]);

        $t2 = new TableBlueprint('users');

        $t2->text('c')->first();
        $sqlList2 = new MariaDbSchemaCompiler()->compileAlterTableAddColumns($t2);

        self::assertSame('ALTER TABLE `users` ADD COLUMN `c` TEXT NOT NULL FIRST', $sqlList2[0]);
    }

    /**
     * Проверяет MariaDB ALTER TABLE MODIFY COLUMN для change().
     */
    #[Test]
    public function mariaDbAlterTableSupportsChangeColumn(): void
    {
        $t = new TableBlueprint('users');

        $t->string('email', 320)
            ->nullable()
            ->default('unknown@example.test')
            ->comment('Email')
            ->after('name')
            ->change();

        $sqlList = new MariaDbSchemaCompiler()->compileAlterTableChangeColumns($t);

        self::assertCount(1, $sqlList);
        self::assertSame(
            "ALTER TABLE `users` MODIFY COLUMN `email` VARCHAR(320) DEFAULT 'unknown@example.test' COMMENT 'Email' AFTER `name`",
            $sqlList[0],
        );
    }

    /**
     * Проверяет MariaDB ALTER TABLE RENAME COLUMN.
     */
    #[Test]
    public function mariaDbAlterTableSupportsRenameColumn(): void
    {
        $t = new TableBlueprint('users');

        $t->renameColumn('display_name', 'name');

        $sqlList = new MariaDbSchemaCompiler()->compileAlterTableRenameColumns($t);

        self::assertSame([
            'ALTER TABLE `users` RENAME COLUMN `display_name` TO `name`',
        ], $sqlList);
    }

    /**
     * Проверяет, что MariaDB использует ALTER TABLE ... DROP INDEX для удаления индексов.
     */
    #[Test]
    public function mariaDbDropIndexUsesAlterTableSyntax(): void
    {
        $t = new TableBlueprint('users');

        $t->dropIndex('users_email_idx');

        $sqlList = new MariaDbSchemaCompiler()->compileDropIndexes($t);

        self::assertCount(1, $sqlList);
        self::assertSame('ALTER TABLE `users` DROP INDEX `users_email_idx`', $sqlList[0]);
    }

    /**
     * Проверяет, что MariaDB использует ALTER TABLE ... DROP FOREIGN KEY для удаления FK.
     */
    #[Test]
    public function mariaDbDropForeignKeyUsesAlterTableSyntax(): void
    {
        $t = new TableBlueprint('users');

        $t->dropForeignKey('users_account_fk');

        $sqlList = new MariaDbSchemaCompiler()->compileAlterTableDropForeignKeys($t);

        self::assertCount(1, $sqlList);
        self::assertSame('ALTER TABLE `users` DROP FOREIGN KEY `users_account_fk`', $sqlList[0]);
    }

    /**
     * Проверяет, что MariaDB useCurrent/useCurrentOnUpdate работает для TIMESTAMP при соответствующем формате.
     */
    #[Test]
    public function mariaDbUseCurrentForTimestamp(): void
    {
        $t = new TableBlueprint('events');

        $t->timestamp('updated_at')
                    ->useCurrent(true, UseCurrentFormatsEnum::TIMESTAMP)
                    ->useCurrentOnUpdate(true, UseCurrentFormatsEnum::TIMESTAMP);

        $sql = new MariaDbSchemaCompiler()->compileCreateTable($t);

        self::assertStringContainsString('`updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP', $sql);
    }

    /**
     * Проверяет, что MariaDB компилирует decimal как DECIMAL(precision,scale).
     */
    #[Test]
    public function mariaDbCompilesDecimalColumn(): void
    {
        $t = new TableBlueprint('products');

        $t->decimal('price', 12, 3);

        $sql = new MariaDbSchemaCompiler()->compileCreateTable($t);

        self::assertStringContainsString('`price` DECIMAL(12,3) NOT NULL', $sql);
    }

    /**
     * Проверяет, что PostgreSQL компилирует decimal как NUMERIC(precision,scale).
     */
    #[Test]
    public function postgresCompilesDecimalColumn(): void
    {
        $t = new TableBlueprint('products');

        $t->decimal('price', 12, 3);

        $sql = new PostgresSchemaCompiler()->compileCreateTable($t);

        self::assertStringContainsString('"price" NUMERIC(12,3) NOT NULL', $sql);
    }

    /**
     * Проверяет PostgreSQL ALTER COLUMN для change().
     */
    #[Test]
    public function postgresAlterTableSupportsChangeColumn(): void
    {
        $t = new TableBlueprint('users');

        $t->string('email', 320)
            ->nullable()
            ->default('unknown@example.test')
            ->change();

        $sqlList = new PostgresSchemaCompiler()->compileAlterTableChangeColumns($t);

        self::assertSame([
            'ALTER TABLE "users" ALTER COLUMN "email" DROP DEFAULT',
            'ALTER TABLE "users" ALTER COLUMN "email" TYPE VARCHAR(320)',
            'ALTER TABLE "users" ALTER COLUMN "email" DROP NOT NULL',
            "ALTER TABLE \"users\" ALTER COLUMN \"email\" SET DEFAULT 'unknown@example.test'",
        ], $sqlList);
    }

    /**
     * Проверяет PostgreSQL ALTER TABLE RENAME COLUMN.
     */
    #[Test]
    public function postgresAlterTableSupportsRenameColumn(): void
    {
        $t = new TableBlueprint('users');

        $t->renameColumn('display_name', 'name');

        $sqlList = new PostgresSchemaCompiler()->compileAlterTableRenameColumns($t);

        self::assertSame([
            'ALTER TABLE "users" RENAME COLUMN "display_name" TO "name"',
        ], $sqlList);
    }

    /**
     * Проверяет, что PostgreSQL игнорирует first()/after(), потому что не поддерживает позиционирование колонок.
     */
    #[Test]
    public function postgresAlterTableIgnoresColumnPositionModifiers(): void
    {
        $t = new TableBlueprint('users');

        $t->text('email')->after('name');

        $sqlList = new PostgresSchemaCompiler()->compileAlterTableAddColumns($t);

        self::assertSame([
            'ALTER TABLE "users" ADD COLUMN "email" TEXT NOT NULL',
        ], $sqlList);
    }

    /**
     * Проверяет PostgreSQL tsvector generated column.
     */
    #[Test]
    public function postgresCompilesGeneratedTsvectorColumn(): void
    {
        $t = new TableBlueprint('products');

        $t->id();
        $t->tsvector('search_vector')->generatedAs(
            PgSearchVectorExpression::make('russian')
                ->column('name', 'A')
                ->column('description', 'B'),
        );

        $sql = new PostgresSchemaCompiler()->compileCreateTable($t);

        self::assertStringContainsString(
            '"search_vector" TSVECTOR GENERATED ALWAYS AS (setweight(to_tsvector(\'russian\', coalesce("name", \'\')), \'A\') || setweight(to_tsvector(\'russian\', coalesce("description", \'\')), \'B\')) STORED NOT NULL',
            $sql,
        );
    }

    /**
     * Проверяет PostgreSQL GIN-индексы по колонке и expression.
     */
    #[Test]
    public function postgresCompilesGinIndexes(): void
    {
        $t = new TableBlueprint('products');

        $t->tsvector('search_vector');
        $t->ginIndex(['search_vector'], 'products_search_vector_gin');
        $t->ginIndexExpression(
            PgSearchVectorExpression::make('simple')->column('technical_markers', 'A'),
            'products_technical_search_gin',
        );

        $sqlList = new PostgresSchemaCompiler()->compileCreateIndexes($t);

        self::assertSame(
            'CREATE INDEX IF NOT EXISTS "products_search_vector_gin" ON "products" USING GIN ("search_vector")',
            $sqlList[0],
        );
        self::assertSame(
            'CREATE INDEX IF NOT EXISTS "products_technical_search_gin" ON "products" USING GIN ((setweight(to_tsvector(\'simple\', coalesce("technical_markers", \'\')), \'A\')))',
            $sqlList[1],
        );
    }

    /**
     * Проверяет PostgreSQL extension helpers в SchemaBuilder.
     */
    #[Test]
    public function postgresSchemaBuilderExecutesExtensionSql(): void
    {
        $conn = new SpyConnection(new FakePdo('pgsql'));

        $builder = new SchemaBuilder($conn, new PostgresSchemaCompiler());

        $builder->createExtensionIfNotExists('unaccent');
        $builder->dropExtensionIfExists('unaccent');

        self::assertCount(2, $conn->executed);
        self::assertSame('CREATE EXTENSION IF NOT EXISTS "unaccent"', $conn->executed[0]['sql']);
        self::assertSame('DROP EXTENSION IF EXISTS "unaccent"', $conn->executed[1]['sql']);
    }

    /**
     * Проверяет, что SQLite компилирует decimal как NUMERIC(precision,scale).
     */
    #[Test]
    public function sqliteCompilesDecimalColumn(): void
    {
        $t = new TableBlueprint('products');

        $t->decimal('price', 12, 3);

        $sql = new SqliteSchemaCompiler()->compileCreateTable($t);

        self::assertStringContainsString('"price" NUMERIC(12,3) NOT NULL', $sql);
    }

    /**
     * Проверяет, что SchemaBuilder::create() выполняет уже два SQL, если в blueprint описан индекс.
     */
    #[Test]
    public function createExecutesIndexSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->create('users', function (TableBlueprint $t): void {
            $t->id();
            $t->string('email', 255)->unique('users_email_unique');
        });

        self::assertCount(2, $conn->executed);
        self::assertSame(
            'CREATE TABLE IF NOT EXISTS "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "email" TEXT NOT NULL)',
            $conn->executed[0]['sql'],
        );
        self::assertSame(
            'CREATE UNIQUE INDEX IF NOT EXISTS "users_email_unique" ON "users" ("email")',
            $conn->executed[1]['sql'],
        );
    }

    /**
     * Проверяет, что внешний ключ добавляется в CREATE TABLE.
     */
    #[Test]
    public function createIncludesForeignKeyConstraints(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->create('user_roles', function (TableBlueprint $t): void {
            $t->bigInteger('user_id');
            $t->bigInteger('role_id');
            $t->foreignKey(['user_id'], 'users', ['id'])->onDelete('cascade');
            $t->foreignKey(['role_id'], 'roles', ['id'])->onDelete('cascade');
        });

        self::assertCount(1, $conn->executed);
        self::assertSame(
            'CREATE TABLE IF NOT EXISTS "user_roles" ("user_id" INTEGER NOT NULL, "role_id" INTEGER NOT NULL, CONSTRAINT "user_roles_user_id_fk" FOREIGN KEY ("user_id") REFERENCES "users" ("id") ON DELETE CASCADE, CONSTRAINT "user_roles_role_id_fk" FOREIGN KEY ("role_id") REFERENCES "roles" ("id") ON DELETE CASCADE)',
            $conn->executed[0]['sql'],
        );
    }

    /**
     * Проверяет, что SchemaBuilder::createIfNotExists() является обёрткой над create(..., true).
     */
    #[Test]
    public function createIfNotExistsExecutesGeneratedSql(): void
    {
        $conn = new SpyConnection(new FakePdo('sqlite'));

        $builder = new SchemaBuilder($conn, new SqliteSchemaCompiler());

        $builder->createIfNotExists('users', function (TableBlueprint $t): void {
            $t->id();
            $t->text('name');
        });

        self::assertCount(1, $conn->executed);
        self::assertSame(
            'CREATE TABLE IF NOT EXISTS "users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT, "name" TEXT NOT NULL)',
            $conn->executed[0]['sql'],
        );
    }
}
