...
use PhpSoftBox\Database\Schema\MissingColumnsResult;
...
#[CoversClass(MissingColumnsResult::class)]
final class SqliteSchemaTest extends TestCase
{
    ...
    public function listsTablesAndColumns(): void
    {
        ...
        self::assertTrue($schema->hasIndex('posts', 'idx_posts_user_id'));
        self::assertFalse($schema->hasIndex('posts', 'missing_idx'));

        $fkByCol = $schema->foreignKeysByColumn('posts', 'user_id');
        self::assertNotEmpty($fkByCol);
        self::assertFalse($schema->hasForeignKey('posts', 'missing_fk'));

        $missing = $schema->missingColumns('users', ['id', 'name', 'missing', 'missing']);
        self::assertSame(['missing'], $missing->all());
        self::assertTrue($missing->has('missing'));
        self::assertFalse($missing->has('id'));
        self::assertTrue($missing->isNotEmpty());
