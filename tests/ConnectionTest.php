...
#[CoversMethod(Connection::class, 'warmup')]
final class ConnectionTest extends TestCase
{
    /**
     * Проверяет, что warmup lookup не удерживает Connection и его PDO после завершения runtime scope.
     *
     * @see Connection::warmup()
     */
    #[Test]
    public function warmupDoesNotRetainConnectionAfterLastExternalReferenceIsReleased(): void
    {
        $garbageCollectorWasEnabled = gc_enabled();
        gc_disable();

        try {
            $pdo = new PDO('sqlite::memory:');

            $pdoReference = WeakReference::create($pdo);
            ...
            self::assertNull($connectionReference->get());
            self::assertNull($pdoReference->get());
        } finally {
            if ($garbageCollectorWasEnabled) {
                gc_enable();
            }
    ...
    /**
     * Проверяет, что разные warmup lookup одного подключения используют общее прогретое состояние.
     *
     * @see Connection::warmup()
     */
    #[Test]
    public function warmupReusesStoreBetweenLookupInstances(): void
    {
        $pdo = new PDO('sqlite::memory:');
        ...
        $connection = new Connection($pdo, new SqliteDriver(), logger: $logger);
        $lookup     = LookupSpec::forTable('users')->lookupColumn('id')->value(1);

        $first  = $connection->warmup()->one($lookup);
        $second = $connection->warmup()->one($lookup);

        self::assertSame(['id' => 1, 'name' => 'Alice'], $first);
    ...
    /**
     * Проверяет, что MariaDB identifiers экранируются backtick-символами, а quoteTable() учитывает prefix.
     */
    #[Test]
    public function quotesIdentifiersAndTablesForMariaDb(): void
    {
    ...
    /**
     * Проверяет, что PostgreSQL identifiers экранируются двойными кавычками, а quoteTable() поддерживает schema.table.
     */
    #[Test]
    public function quotesIdentifiersAndTablesForPostgres(): void
    {
    ...
    /**
     * Проверяет, что write-запросы очищают локальный warmup store.
     */
    #[Test]
    public function executeClearsWarmupStore(): void
    {
        $store = new WarmupStore();
        $key   = WarmupKey::single('main', 'users', 'id', 1);
        $store->set($key, WarmupEntry::row(['id' => 1]));

        $conn = new Connection(
            new PDO('sqlite::memory:'),
            new SqliteDriver(),
            warmupConnectionName: 'main',
            warmupStore: $store,
        );

        self::assertNotNull($store->get($key));
