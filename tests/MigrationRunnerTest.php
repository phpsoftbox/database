...
{
    /**
     * Проверяет, что applied_datetime передаётся как DateTimeInterface, а не как ISO-строка.
     *
     * Это позволяет Connection форматировать значение под конкретный драйвер:
     * MariaDB DATETIME принимает "Y-m-d H:i:s", а не DATE_ATOM.
     */
    #[Test]
    public function markAppliedPassesDateTimeParameterToConnection(): void
    {
        $connection = new SpyConnection(new FakePdo('mysql'));
        $repository = new SqlMigrationRepository('migrations');

        $repository->markApplied($connection, '20260608120000_create_users_table', 'main');

        $insert = null;
        foreach ($connection->executed as $query) {
            if (str_contains($query['sql'], 'INSERT INTO migrations')) {
                $insert = $query;
                break;
            }
        }
        ...
        self::assertArrayHasKey('applied_datetime', $insert['params']);
        self::assertInstanceOf(DateTimeImmutable::class, $insert['params']['applied_datetime']);
    }
